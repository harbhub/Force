...
var Game = function Game(config) {
	// Pre-Build
	var game = this;
	game.cards = ['Bronze', 'Silver', 'Gold', 'Swordsman', 'Archer', 'Jouster', 'Shield', 'Brainstorm', 'Cycle', 'Demolition', 'Harvest', 'Swindle', 'Melting Pot', 'Bazaar', 'Laboratory', 'Mercenary'];
	...
	function buildPlayers() {
		for (var i = 0; i < game.players.length; ++i) {
			...
			game.player[name] = {
				...
				deck: ['Bronze', 'Bronze', 'Swordsman'].shuffle(),
				hand: [],
				discard: [],
				field: [],
				selectedFromHand: [],
	...
	function buildShop() {
		for (var i = 0; i < game.cards.length; ++i) {
			var name = game.cards[i];
			var card = cards[name];
			if (name === 'Bronze') {
				game.shop[name] = 60 - game.players.length * 7;
				continue;
			} else if (name === 'Silver') {
				game.shop[name] = 40;
				continue;
				...
				game.shop[name] = game.players.length > 2 ? 12 : 8;
				continue;
	...
	function play(name) {
		var player = this;
		console.log('play', name, 'player', player);
